...
class UserUpdateForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            user.address, created = UserAddress.objects.get_or_create(user=user) # if account exists then get if not then create

            user.account.account_type = self.cleaned_data['account_type']
            user.account.gender = self.cleaned_data['gender']
            user.account.birth_date = self.cleaned_data['birth_date']
            user.account.save()

            
            user.address.street_address = self.cleaned_data['street_address']
            user.address.city = self.cleaned_data['city']
            user.address.postal_code = self.cleaned_data['postal_code']
            user.address.country = self.cleaned_data['country']
            user.address.save()
